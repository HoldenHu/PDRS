...
from model.KnowledgeMLP import KnowledgeMLP
...
def train(k_model, k_opt, k_criterion, k_train_loader, k_test_loader, y_model, y_opt, y_criterion, train_loader, validation_loader, test_loader, config):
    ...
    for epoch in range(config.getint('MODEL', 'epoch')):
        # Train K
        k_model.embed_k_MLP.weight.data.copy_(y_model.embed_k_MLP.weight)
        k_model.train()
        k_total_loss = 0
        ...
        # Train Y
        y_model.embed_k_MLP.weight.data.copy_(k_model.embed_k_MLP.weight)
        y_model.train()
        y_total_loss = 0
...
def main():
    ...
    for config,var in zip(config_list, variant_list):
        ...
        n_knowledge = data_splitter.n_knowledge
        n_user = data_splitter.n_user
        n_item = data_splitter.n_item

        import gc
        del data_splitter
        gc.collect() # release memory by garbage collector
        ...
        kdropout = config.getfloat('MODEL','kdropout')
        num_layers = config.getint('MODEL','num_layers')
        knum_layers = config.getint('MODEL','knum_layers')
        ...
        use_pretrain = config.getboolean('MODEL','use_pretrain')
        result_dir = generate_result_path(config, pretrain=use_pretrain)
        if not os.path.exists(result_dir):
            '''
            START MODEL TRAINING
            '''
            print('> [main]<K model setting> kbatch_size = {}, klr = {}, klatent_dim = {}, kl2_reg = {}'.format(kbatch_size, klr, klatent_dim, kl2_reg))
            k_model = KnowledgeMLP(n_knowledge, klatent_dim, num_layers=knum_layers, dropout=kdropout).to('cuda:0')
            if use_pretrain:
                _rs_config = copy.deepcopy(config)
                _rs_config['MODEL']['method'] = 'KnowledgeMLP'
                knowledgemlp_result_dir = generate_result_path(_rs_config)
                k_model.load_state_dict(torch.load(os.path.join(knowledgemlp_result_dir, 'model.pth')))
                print('> [main] Loaded pretrained K model from',knowledgemlp_result_dir)

            k_opt = optim.Adam(k_model.parameters(), lr=klr, weight_decay=kl2_reg)
            k_criterion = nn.MSELoss()

            print('> [main]<Y model setting> batch_size = {}, lr = {}, latent_dim = {}, l2_reg = {}'.format(batch_size, lr, latent_dim, l2_reg))
            y_model = KDMLP(n_user, n_item, n_knowledge, latent_dim, klatent_dim, num_layers=num_layers, dropout=dropout, use_priork=use_priork, use_targetk=use_targetk, use_itemk=use_itemk).to('cuda:0')
            if use_pretrain:
                _rs_config = copy.deepcopy(config)
                _rs_config['MODEL']['method'] = 'MLP'
                _rs_config['MODEL']['latent_dim'] = str(int(int(_rs_config['MODEL']['latent_dim'])/2))
                mlp_result_dir = generate_result_path(_rs_config)

                _y_model = MLP(n_user, n_item, _rs_config.getint('MODEL','latent_dim'), num_layers=_rs_config.getint('MODEL','num_layers'), dropout=_rs_config.getfloat('MODEL','dropout'))
                _y_model.load_state_dict(torch.load(os.path.join(mlp_result_dir, 'model.pth')))
                print('> [main] Loaded pretrained Y model from',mlp_result_dir)
                y_model.embed_user_MLP.weight.data.copy_(_y_model.embed_user_MLP.weight)
                y_model.embed_item_MLP.weight.data.copy_(_y_model.embed_item_MLP.weight)
                print('> [main] Copied the weight from the pretrained Y model to KDMLP')
            ...
            if config.getboolean('OUTPUT','save_result'):
                ...
                print('> [end] saved result to:',result_dir)
            '''
            END MODEL TRAINING
            '''
